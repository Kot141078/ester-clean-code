...
- Added: integratsiya s cards (svyazi pri dobavlenii faktov)
- Added: primenenie decay ko vsemu grafu"""
...
DECAY_FACTOR = 0.99  # The decay rate of the connection weight over time
...
class EsterKnowledgeGraph:
    ...
    def load_graph(self):
        if os.path.exists(self.storage_path):
            try:
                # In modern versions of Networks it is better to use Nodier-link data for ZhSON
                with open(self.storage_path, 'r', encoding='utf-8') as f:
                    data = json.load(f)
                    self.graph = nx.node_link_graph(data)
            except Exception as e:
                print(f"Error loading graph: ZZF0Z")
                self.graph = nx.MultiDiGraph()
    ...
    def add_relation(self, source: str, target: str, relation_type: str, weight: float = 1.0):
        """Add a connection (edge) between entities."""
        self.graph.add_edge(
            source, 
            target, 
            key=relation_type,
    ...
    # Improved: Method for deleting a node
    def remove_node(self, node_id: str):
        if node_id in self.graph.nodes:
            self.graph.remove_node(node_id)

    # Improved: Search for associations (neighboring nodes)
    ...
    def find_path(self, start_node: str, end_node: str):
        """Find the shortest path between two concepts (associative chain)."""
        try:
