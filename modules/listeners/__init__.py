...
where lezhit realnaya papka `listeners/`.
...
My podnimaemsya vverkh po derevu i addavlyaem put, where est `listeners/`.
# c=a+b"""
...
def _import_listeners():
    ...
    try:
        ...
        pass
    # 2) Search up from the current file
    ...
    for parent in list(here.parents)[:8]:  # podnimaemsya do 8 urovney
        ...
        if (parent / "listeners").is_dir():
            ...
            if root not in sys.path:
                sys.path.insert(0, root)
            return importlib.import_module("listeners")
    # 3) Final fullback: informational error
