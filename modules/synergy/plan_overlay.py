# -*- coding: utf-8 -*-
"""modules/synergy/plan_overlay.py - “overley” sovetnika poverkh proizvolnogo plana (only metadannye).

MOSTY:
- (Yavnyy) build_overlay(plan, extras, alpha) → dictionary metadannykh k kandidatam: bias, labels, why, synergy_avg, color_hint.
...
chtoby operator bystree prinyal vzveshennoe decision.
...
def _find_candidates_in_plan(plan: Dict[str, Any]) -> List[str]:
    # Find agent_id inside "candidates": yZF0Z, ...sch for all steps
    ids: List[str] = []
...
def _color_from_bias_synergy(bias: float, syn: float) -> str:
    """Returns the color recommendation for the UI (light heart rate):
      b++b / b+b / b0b / b-b - the stronger the green, the clearer the hint."""
    # normalizuem v [-1..+1]
