...
def from_file(path: str, prefer_lang: str | None = None) -> Dict[str,Any]:
    ...
    # attempt to extract the first subtitle track
    try:
        args = [FFMPEG, "-y", "-i", path, "-map", "0:s:0", out]
        if prefer_lang:
            # filter by language (best-effort): ffmpeg will select the first corresponding language
            args = [FFMPEG, "-y", "-i", path, "-map", "0:s:m:language:"+prefer_lang, out]
        r = subprocess.run(args, capture_output=True, text=True, check=False)
        if r.returncode != 0:
            # didn't work - maybe there are no subtitles
...
def from_url(url: str, prefer_lang: str | None = None) -> Dict[str,Any]:
    ...
    # from-dlp usually puts .srt next to it; we'll find
    ...
    if not candidates:
        # let's try to extract from the file
