...
def _candidate_fast(task: str) -> Dict[str, Any]:
    return {"name":"fast","text":f"Korotkiy otvet: {task.strip()} — sdelay minimum, prover, iteratsiya za 24 chasa.","score": 0.0}

def _candidate_careful(task: str) -> Dict[str, Any]:
    return {"name":"careful","text":f"Akkuratnyy plan po shagam dlya: {task.strip()}.\n1) Sbor dannykh\n2) Riski\n3) Deystviya\n4) Proverka","score": 0.0}

def _candidate_tools(task: str) -> Dict[str, Any]:
    return {"name":"tool-aug","text":f"Ispolzuy instrumenty: chek-list, shablon pisma, tablitsu progressa — zadacha: {task.strip()}.","score": 0.0}
...
def run_branches(task: str, k: int = 3) -> Dict[str, Any]:
    """
    Vozvraschaet {candidates, pick, sketch}.
    Slot A: 3 bazovye strategii. Slot B: dobavlyaem esche «risk-cut» i «expand».
    """
    task = (task or "").strip()
    if not task:
    ...
    with ab_switch("BRANCH") as slot:
        ...
        if slot == "B":
            cand.append({"name":"risk-cut","text":f"Srez riskov dlya: {task}. Srazu snimem izvestnye tochki otkaza i dobavim rezerv.", "score":0.0})
            cand.append({"name":"expand","text":f"Rasshirenie gorizonta: {task}. Rassmotrim 2–3 alternativnykh puti i sdelaem ToT-skhemu.", "score":0.0})
        if cc._v2_enabled():
