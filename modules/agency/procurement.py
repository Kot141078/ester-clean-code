...
reuse → free/open-source → obmen s sestrami → zarabotat → kupit.

Mosty:
- Yavnyy: (Poisk ↔ Inzhest) ischem besplatnye analogi i srazu gotovim URL dlya inzhesta.
...
Zemnoy abzats:
Eto kak chek-list mastera: snachala poprobuy tem, chto uzhe est, zatem besplatnye instrumenty, zatem obmen, tolko potom dengi.
...
def plan_need(need: str, budget_eur: float = 0.0) -> Dict[str, Any]:
    ...
    steps.append({"kind":"reuse.scan", "explain":"proverit uzhe podklyuchennye rasshireniya i gotovye funktsii", "status":"plan"})
    ...
    # 4) zarabotat: podgotovit bezopasnye zadachi (draft), ne publikovat bez operatora
    earn = {
        "kind":"earn.draft",
        "explain":"sformirovat spisok malenkikh zadach na 1–2 chasa (dok, testy, melkie fiksy), kotorye mozhno vypolnit dlya dokhoda",
        "tasks":[
            {"title":"dobavit avtotesty k video-payplaynu", "est_reward_eur": 15},
            {"title":"napisat korotkiy gayd po podklyucheniyu SSA/ASS", "est_reward_eur": 10},
        ],
        "status":"plan"
    }
    steps.append(earn)

    # 5) kupit (esli nado i khvataet kapov/balansa)
    if budget_eur > 0:
        pill = _pill_armed()
        allow = L.spend_allowed(budget_eur, DAILY_CAP, MONTHLY_CAP, pill)
        steps.append({"kind":"buy.check", "explain":"proverka limitov dnevnykh/mesyachnykh i statusa 'tabletki'", "status":"plan", "caps": allow})
...
def execute(plan: Dict[str, Any]) -> Dict[str, Any]:
    """
    Bezopasnoe ispolnenie: delaem tolko to, chto ne obraschaetsya k dengam.
    ...
    for st in plan.get("steps", []):
        ...
        if st.get("kind") == "buy.intent" and st.get("allowed_now"):
            # fiksatsiya «raskhoda» kak rezerva (bez realnoy pokupki)
