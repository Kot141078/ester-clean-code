...
- VIDEO_INGEST_AB=A (po umolchaniyu): ASR cherez suschestvuyuschiy dvizhok asr_engine.asr_transcribe (drop-in).
...
- Skrytyy #1: (Kibernetika ↔ Planirovschik) rezultaty atomarno registriruyutsya v ingest-dedup indekse, prigodny dlya paketnoy/f fonovoy obrabotki.
...
delaet kratkiy raport, kladovschik (memory) fiksiruet uchet. Vse vosproizvodimo i s zaschitoy ot dubley.
...
# Dedup i obschiy indeks (ideomatichno dlya suschestvuyuschego ingest)
try:
...
# NLP/myshlenie: myagkaya integratsiya
def _maybe_summarize(text: str) -> str:
    """
    Pytaetsya zapustit thinking_pipelines.run_rules dlya summarizatsii.
    Folbek — chastotnye klyuchevye slova iz modules.text_analyzer.
    """
    text = (text or "").strip()
    if not text:
        return ""
    # Popytka cherez thinking_pipelines
    try:
        ...
        pass
    # Folbek: prostoy «konspekt» cherez klyuchevye slova
    try:
        ...
        for w in kws:
            ...
            if len(uniq) >= 50:
                break
        return "Klyuchevye temy: " + ", ".join(uniq)
    except Exception:
        return text[:1000]
...
def _asr_file(path_wav: str) -> Tuple[str, List[Dict[str, Any]], str]:
    ...
    # A — deystvuyuschiy dvizhok asr_engine (drop-in)
    if ab == "A":
        try:
            ...
            ab = "B"  # avto-otkat
    # B — faster-whisper, esli dostupna
    try:
        ...
        # finalnyy folbek — oshibka naruzhu
...
def _merge_subs_or_asr(subs_paths: List[str], asr_text: str, asr_segments: List[Dict[str, Any]]) -> Tuple[str, List[Dict[str, Any]]]:
    """
    Esli est srt — ispolzuem kak pervichnyy transkript (ploskiy tekst + naivnye segmenty),
    inache vozvraschaem ASR-rezultat.
    """
    if subs_paths:
        buf: List[str] = []
...
def ingest_video(
    ...
    """
    Universalnaya tochka vkhoda: prinimaet URL ili lokalnyy put.
    ...
    if want_transcript:
        ...
        # Prostaya posledovatelnaya sborka (sovmestimo s suschestvuyuschim planirovschikom, mozhno rasparallelit pozzhe)
    ...
    # 8) Sokhranenie v pamyat/khranilische (best-effort, myagkie importy)
    saved: Dict[str, Any] = {}
    try:
        ...
        pass
    # Dobavim v KG, esli est API commons (kak v ingest text/pdf)
    try:
    ...
    # Polozhim kopiyu rezultata v PERSIST_DIR dlya inspektsii
    try:
        import time as _t
