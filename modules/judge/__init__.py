...
- Skrytyy #1: (Infoteoriya ↔ Bayes) — prostaya evristika (dlina+raznoobrazie) vmesto pustogo mesta.
...
def select_best(candidates: List[str]) -> str:
    ...
    # prostaya evristika: maksimalnaya dlina
    return max(candidates, key=lambda s: (len(s), s))

def synthesize(candidates: List[str]) -> str:
    if not candidates: return ""
    # "sintez" = luchshaya + khvost iz otlichayuschikhsya korotkim konkatenatom
