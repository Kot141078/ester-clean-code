...
- Skrytyy #2: (Audit ↔ Memory) sobytiya vydachi/proverki mozhno pisat v pamyat cherez passport (po mestu vyzova).
...
def issue(pattern: str, method: str = "POST", ttl: int = 300, subject: str | None = None) -> Dict[str,Any]:
    """
    pattern: regex marshruta; method: ozhidaemyy HTTP-metod; ttl: sek.
    """
    _ensure()
...
def verify(token: str, pattern: str | None = None, method: str | None = None, subject: str | None = None, consume: bool = True) -> Dict[str,Any]:
    """
    Proverka tokena. Esli consume=True — odnorazovo (uspeshnaya proverka udalyaet token).
    """
    ...
    # metod dolzhen sovpast, esli zadan
    if method and t.get("method","") != method.upper():
        return {"ok": False, "error":"method_mismatch"}
    # pattern dolzhen matchitsya s ozhidaemym/fakticheskim
    ...
    try:
        ...
        pass
    # subekt (esli ustanovlen pri vydache), mozhet byt pustym
