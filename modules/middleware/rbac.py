...
def _subject_from_request(flask_request) -> str:
    # pytaemsya dostat subject iz zagolovkov; bezopasnyy defolt — anonymous
    sub = _header_utf8(flask_request, "X-Subject")
    return sub or "anonymous"


def _header_utf8(flask_request, name: str) -> str:
    base = str(name or "").strip()
    if not base:
        return ""
    b64 = flask_request.headers.get(base + "-B64", "")
    text = _b64url_decode_utf8(b64)
    if text:
        return text
    return str(flask_request.headers.get(base, "") or "")
