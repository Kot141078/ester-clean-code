...
function Enable-Target([string]$RelativePath) {
  ...
  if (Test-Path -LiteralPath $active) {
    Write-Host "[OK] already ON: $RelativePath"
    return
  }

  if (Test-Path -LiteralPath $off) {
    Ensure-WriteAccess $off
    Rename-Item -LiteralPath $off -NewName ([System.IO.Path]::GetFileName($active)) -Force
    Write-Host "[ON] $RelativePath"
    return
  }
