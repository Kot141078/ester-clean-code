...
def main() -> int:
    tmp_root = Path(tempfile.mkdtemp(prefix="ester_drift_quarantine_smoke_")).resolve()
    ...
    try:
        create = agent_factory.create_agent(
            {
                "name": "agent_drift_quarantine_smoke",
                "goal": "quarantine enforcement smoke",
                "template_id": "builder.v1",
                "capabilities": ["cap.fs.sandbox.write"],
                "owner": "tools.drift_quarantine_smoke",
        ...
        q_set = drift_quarantine.ensure_quarantine_for_agent(agent_id, source="smoke")
        ...
        enqueue_block = action_registry.invoke(
            "agent.queue.enqueue",
            {
                "agent_id": agent_id,
                "actor": "ester:smoke",
                "reason": "drift_quarantine_block",
                "challenge_sec": 0,
                "plan": {
                    "schema": "ester.plan.v1",
                    "plan_id": "drift_quarantine_smoke_block",
        ...
        clear_args = {
            ...
            "by": "tools.drift_quarantine_smoke",
            "chain_id": "chain_drift_quarantine_smoke_clear",
            "evidence": dict(clear_evidence),
            "evidence_note": "smoke evidence",
        ...
        clear_rep = action_registry.invoke_guarded("drift.quarantine.clear", clear_args)
        ...
        enqueue_after = action_registry.invoke(
            "agent.queue.enqueue",
            {
                "agent_id": agent_id,
                "actor": "ester:smoke",
                "reason": "drift_quarantine_after_clear",
                "challenge_sec": 0,
                "plan": {
                    "schema": "ester.plan.v1",
                    "plan_id": "drift_quarantine_smoke_after",
        ...
        ok = (
            ...
            and str(q_set.get("severity") or "") == "HIGH"
            ...
            and ("QUARANTINE_SET" in event_types)
            and ("QUARANTINE_BLOCK" in event_types)
            and ("QUARANTINE_CLEAR" in event_types)
        )
