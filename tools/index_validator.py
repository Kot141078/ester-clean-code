...
Chto proveryaet:
  ...
  2) Sootvetstvie razmera ids i fakticheskogo chisla vektorov (po faylam).
...
Eto «indikator bieniya shpindelya»: bystrym prokhodom ubezhdaemsya, chto indeksy zhivye,
...
# optsionalno numpy; esli net — ogranichimsya strukturnoy proverkoy
try:
...
def _embed_texts(texts, dim: int):
    # lokalnyy determinirovannyy embedder (kak v /index/build)
    import hashlib
    if np is None:
...
def main():
    ...
    # Sempliruem teksty i proveryaem poisk (esli est numpy)
    if np is not None:
        # soberem teksty
        l0 = list(_iter_l0())
        texts = []
        ...
        texts = texts[:args.samples] if texts else ["proverka poiska", "teoriya informatsii", "bayesovskiy podkhod", "entropiya signala"]
        X = _embed_texts(texts, dim=args.dim)

        for s in shards:
            idx = _index(s, dim=args.dim, backend=args.backend)
            # lenivaya zagruzka proizoydet vnutri search
            try:
