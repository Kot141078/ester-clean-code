...
def backup(p: Path) -> Path:
    ...
    b = p.with_suffix(p.suffix + f".bak_A2f_{ts}")
...
_log = logging.getLogger("ester.chroma_ui")
...
def _infer_vstore_root(persist_dir: str) -> Optional[str]:
    """
    Try infer ESTER_VSTORE_ROOT from ...\\vstore\\chroma
    """
    try:
        ...
        return None
    try:
        if p.name.lower() == "chroma" and p.parent.name.lower() == "vstore":
            return str(p.parent)
    except Exception:
        return None
    return None

def _maybe_autofix_process_env(primary_dir: str, warnings: List[str]) -> Dict[str, Any]:
    """
    Safe auto-heal: only set env vars INSIDE current process.
    No file moves.
    """
    report: Dict[str, Any] = {"changed": False, "chroma_persist_dir": None, "ester_vstore_root": None}

    if not primary_dir:
        return report

    enabled = (_env("CHROMA_AUTO_HEAL_ENV", "1").lower() in ("1","true","yes","on"))
    if not enabled:
        return report

    cur = os.getenv("CHROMA_PERSIST_DIR", "")
    need = (not cur) or _looks_unexpanded(cur) or _looks_unexpanded(os.path.expandvars(cur))
    if need:
        os.environ["CHROMA_PERSIST_DIR"] = str(primary_dir)
        report["changed"] = True
        report["chroma_persist_dir"] = str(primary_dir)
        warnings.append("auto_heal_env: set CHROMA_PERSIST_DIR for this process (was empty/unexpanded).")
        try:
            _log.warning("auto_heal_env: CHROMA_PERSIST_DIR -> %s", primary_dir)
        except Exception:
            pass

    # Optionally infer ESTER_VSTORE_ROOT (process-only), if empty/unexpanded
    vr = os.getenv("ESTER_VSTORE_ROOT", "")
    vr_need = (not vr) or _looks_unexpanded(vr) or _looks_unexpanded(os.path.expandvars(vr))
    inf = _infer_vstore_root(primary_dir)
    if inf and vr_need:
        os.environ["ESTER_VSTORE_ROOT"] = inf
        report["changed"] = True
        report["ester_vstore_root"] = inf
        warnings.append("auto_heal_env: set ESTER_VSTORE_ROOT for this process (inferred).")
        try:
            _log.warning("auto_heal_env: ESTER_VSTORE_ROOT -> %s", inf)
        except Exception:
            pass
...
class MultiChromaUI:
    ...
    Also: safe auto-heal env pointers in-process if unexpanded %VAR% detected.
    """
    def __init__(self) -> None:
        ...
        self._autofix: Dict[str, Any] = {"changed": False}
        ...
        scan_on = (_env("CHROMA_UI_SCAN", "1").lower() in ("1","true","yes","on"))
        if scan_on:
        ...
        # warnings + auto-heal env pointers (process-only)
        try:
        ...
        if self.primary is not None:
            try:
                self._autofix = _maybe_autofix_process_env(self.primary.persist_dir, self._warnings)
            except Exception:
                self._autofix = {"changed": False}
    ...
    def autofix_report(self) -> Dict[str, Any]:
        return dict(self._autofix or {"changed": False})
...
try:
    ...
    raise SystemExit(f"A2f failed, rolled back: {e}")

print("OK: ITER A2f applied")
