...
foreach ($m in $need) { if (-not ($arr -contains $m)) { $arr += $m } }

$arr = $arr | Where-Object { $_ -ne "app_plugins.after_response_sanity" }
$arr += "app_plugins.after_response_sanity"

($arr | ConvertTo-Json -Compress -Depth 4) | Out-File -Encoding UTF8 -LiteralPath $extra
Write-Host ("OK: wrote {0} entries to {1}" -f $arr.Count, $extra)
