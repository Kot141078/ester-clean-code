...
from growth_engine_ester.signals import record_outcome  # noqa: E402


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Record a bounded SRLM real-fitness outcome.")
    parser.add_argument("--source", required=True, choices=["human", "reality", "l4", "model", "judge"])
    parser.add_argument("--kind", required=True, help="Outcome event_kind, e.g. human.answer.corrected")
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = record_outcome(payload, root=args.root)
