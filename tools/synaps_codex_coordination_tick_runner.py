"""Bounded SYNAPS Codex coordination tick runner CLI."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(description="Run bounded SYNAPS Codex coordination session ticks.")
    parser.add_argument("--env-file", default=".env")
    parser.add_argument("--queue-root", default=str(DEFAULT_CODEX_COORDINATION_SESSION_PLAN_QUEUE))
    parser.add_argument("--completed-root", default=str(DEFAULT_CODEX_COORDINATION_SESSION_PLAN_COMPLETED))
    parser.add_argument("--failed-root", default=str(DEFAULT_CODEX_COORDINATION_SESSION_PLAN_FAILED))
    parser.add_argument("--tick-ledger", default=str(DEFAULT_CODEX_COORDINATION_SESSION_TICK_LEDGER))
    parser.add_argument("--runner-ledger", default=str(DEFAULT_CODEX_COORDINATION_TICK_RUNNER_LEDGER))
    ...
    parser.add_argument("--confirm", default="", help=f"Required: {CODEX_COORDINATION_TICK_RUNNER_CONFIRM_PHRASE}")
    parser.add_argument("--max-ticks", type=int, default=1)
    parser.add_argument("--max-runtime-sec", type=float, default=300.0)
    parser.add_argument("--sleep-sec", type=float, default=0.0)
    parser.add_argument("--continue-on-no-work", action="store_true")
    parser.add_argument("--postcheck-root", action="append", default=[])
    args = parser.parse_args(raw_argv)

    env = merged_env(args.env_file)
    base_policy = CodexCoordinationTickRunnerPolicy.from_env(env)
    policy = CodexCoordinationTickRunnerPolicy(
        max_ticks=args.max_ticks,
        max_runtime_sec=args.max_runtime_sec,
        sleep_sec=args.sleep_sec,
        continue_on_no_work=args.continue_on_no_work,
    ...
    payload = run_codex_coordination_tick_runner(
        ...
        tick_ledger_path=args.tick_ledger,
        runner_ledger_path=args.runner_ledger,
