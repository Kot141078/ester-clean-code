...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--input", type=str, default="", help="path to ZhSON with hints (or for stdin)")
    ap.add_argument("--out", type=str, default="", help="kuda sokhranit (defolt ESTER_JUDGE_SLOTS_PATH)")
    ap.add_argument("--bench", action="store_true", help="if --input is not specified, generate with benham")
    ...
    path = save_overlay(build_suggestion(bench=False), path=args.out or None)  # save a new cast (bench optional)
    # If you need strictly “as in c”, you could write c directly; here keeps the fresh build for consistency
