#!/usr/bin/env bash
set -euo pipefail

BASE="${ESTER_BASE_URL:-http://127.0.0.1:5000}"
JWT="${ESTER_JWT:-}"
RPS="${K6_RPS:-10}"
DUR="${K6_DURATION:-2m}"
VUS="${K6_VUS:-20}"
INFLUX_URL="${INFLUX_URL:-http://127.0.0.1:8086/k6}"
ART="artifacts/perf"
mkdir -p "$ART"

if ! command -v k6 >/dev/null 2>&1; then
  echo "[bench-influx] k6 ne nayden. Ustanovi lokalno (scripts/k6_install_local.sh) ili ispolzuy docker k6." >&2
  exit 2
fi

export ESTER_BASE_URL="$BASE" ESTER_JWT="$JWT" K6_DURATION="$DUR" K6_VUS="$VUS" K6_RPS="$RPS"
export K6_OUT="influxdb=${INFLUX_URL}"

echo "[bench-influx] K6_OUT=$K6_OUT"

k6 run --summary-export "$ART/read.summary.json" tests/perf/k6_read.js
k6 run --summary-export "$ART/replicate.summary.json" tests/perf/k6_replicate.js
k6 run --summary-export "$ART/ops.summary.json" tests/perf/k6_ops.js
k6 run --summary-export "$ART/ingest.summary.json" tests/perf/k6_ingest.js

python3 scripts/perf_aggregate.py || true

echo "[bench-influx] Gotovo. Grafana: http://localhost:3000 (admin/admin)"
