...
$dsts = @(
  (Join-Path $root "sitecustomize.py"),
  (Join-Path $root "tools\sitecustomize.py")
)
$payload = @'
...
'@
if ($Mode -eq "A") { $dsts | % { Write-Host "[plan] -> $_" }; exit 0 }
foreach($p in $dsts){ Write-FileUtf8 -Path $p -Content $payload; Write-Host "[ok] wrote $p" }
