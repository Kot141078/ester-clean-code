...
MOSTY:
- (Yavnyy) analyze_and_requeue(flush_start_ts) skaniruet outbox s momenta flush i sozdaet pending s due po raspisaniyu.
...
- (Skrytyy #2) Dlya obkhoda UNIQUE(event_id,key,kind,intent) retrai ssylayutsya na novyy event 'NudgesRetry' (event_id otlichaetsya).

ZEMNOY ABZATs:
Esli soobschenie «ne ushlo» iz-za 503/429 i t.p., my ne zabyvaem — myagko poprobuem snova cherez 5 minut, potom cherez 30 minut, potom cherez 2 chasa.
...
def analyze_and_requeue(flash_start_ts: float) -> int:
    """
    Vozvraschaet kolichestvo postavlennykh retraev.
    """
    codes = set(_codes())
    ...
    with _nudges_conn() as c:
        # 1) zaberem neuspeshnye autboksy s momenta flush
        rows = c.execute("""
        ...
        for ts, key, text, status, http in rows:
            # 2) naydem log nudges, chtoby ponyat kind/event_id
            log = c.execute("""
            ...
            # 3) esli uzhe est pending po etomu klyuchu/intentu — ne dubliruem
            if _has_pending_for(str(key), str(text)):
                continue
            # 4) stupen retraya po chislu proshlykh feylov
            idx = min(len(sched)-1, max(0, _fails_count(str(key), str(text))-1))
            due_ts = now + sched[idx]*60

            # 5) sozdadim synthetic event 'NudgesRetry' s entity prezhnego sobytiya (esli dostupen)
