#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""R3/tests/r3_smoke.py - myagkiy smouk R3 (indexes → zapros) na lokalnykh kartochkakh.

Mosty:
- Yavnyy: Enderton — proveryaem dva predikata: indeks postroen ∧ iz topa vernulsya spisok.
- Skrytyy #1: Cover & Thomas — dostatochen odin prostoy zapros dlya obnaruzheniya classa oshibok TF-IDF.
- Skrytyy #2: Ashbi — regulyator prosche: stdlib-only, nikakikh vneshnikh servisov.

Zemnoy abzats:
Esli kartochek net - ne valim payplayn (pechataem preduprezhdenie). Inache construction indexes i vypolnyaem zapros.
...
def main() -> int:
    n = reco_build()
    if n == 0:
        print("yR3sch VARN: no cards for the index - skipping the request.")
        return 0
