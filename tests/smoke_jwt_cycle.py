...
Mosty:
- Yavnyy: Cover & Thomas (infoteoriya) — minimalnyy «testovyy signal» (odin token) dostatochen, chtoby detektirovat klass oshibok.
...
def main() -> int:
    ...
    if not os.environ.get("JWT_SECRET"):
        if _have("tools/gen_jwt_secret.py"):
            print("[smoke_jwt_cycle] JWT_SECRET ne zadan, sgeneriruyu vremennyy (pamyat protsessa).")
            ...
            if proc.returncode == 0 and out.strip():
                ...
                print("[smoke_jwt_cycle] WARN: ne udalos sgenerirovat sekret, zavershayu myagko.")
                return 0
        else:
