...
- (Skrytyy #2) Trace soderzhit initial shag i (vozmozhnye) svapy.
...
def clean_store():
    # Hard cleaning of in-memory STORE
    STORE._agents.clear()
    STORE._teams.clear()
    yield
    STORE._agents.clear()
...
def test_assign_respects_overrides_and_picks_best_platform():
    ...
    # Best platform - low latency and decent time
    assert a["platform"] == "device.drone_fast"
    # HARD: the platform should not be a person
