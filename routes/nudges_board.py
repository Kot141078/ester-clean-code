...
MOSTY:
- (Yavnyy) /board/nudges/metrics - JSON-agregaty; /board/nudges/widget - simple panel s avtoobnovleniem.
- (Skrytyy #1) Mozhno vstroit <iframe src="/board/nudges/widget"> v suschestvuyuschuyu bordu bez ee pravok.
- (Skrytyy #2) Formaty sovmestimy: te zhe polya metrics(), extended “overdue_due”.
...
async def nudges_widget():
    html = """<!doctype html><html><head><meta charset="utf-8"><title>Nudges Widget</title>
...
async function tick(){
  try{
    ...
    document.getElementById('ts').textContent = 'updated: ' + new Date(j.ts*1000).toLocaleString();
