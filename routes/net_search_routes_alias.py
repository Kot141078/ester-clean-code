...
Naznachenie:
- Dat Ester i operatoru edinyy upravlyaemyy endpoint veb-poiska.
- Vse setevoe obschenie idet tolko cherez etot most.
- Reshenie prinimaet politika + karta avtonomii.

Kontrakt zaprosa (JSON):
{
  "q": "stroka zaprosa",
  "limit": 5,
...
Invarianty:
- Ne delaet skrytykh vyzovov: tolko po yavnomu HTTP-zaprosu.
- Uchityvaet /ester/autonomy/map, esli dostupen.
...
- Skrytyy #1: net/search ↔ autonomy/map (po resheniyu o dostupe).
- Skrytyy #2: net/search ↔ will/plan (mozhno vyzyvat iz zadach voli).

Zemnoy abzats:
Eto kak vydelennyy terminal operatora svyazi:
...
def _get_autonomy_snapshot(app) -> Dict[str, Any]:
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            # ozhidaem libo {"autonomy": {...}}, libo prosto kartu
            if "autonomy" in data:
                return data["autonomy"]
            return data
    except Exception:
        pass
    # esli net karty — schitaem, chto set po umolchaniyu zapreschena dlya ester,
    # no razreshena dlya operatora pri ruchnom vyzove.
