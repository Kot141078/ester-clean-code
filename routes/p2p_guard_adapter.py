...
try:
    # Ispolzuem nash unifitsirovannyy verifikator
...
def _is_enabled() -> bool:
    # A — vklyucheno (po umolchaniyu), B — vyklyucheno (bystryy A/B otkat, esli vdrug ponadobitsya)
    return (os.getenv("P2P_GUARD_MODE", "A") or "A").upper() != "B"

def _secret_present() -> bool:
    # Esli sekret ne zadan — podpis ne trebuetsya (sovmestimo s tekuschim povedeniem)
...
def p2p_guard_before_request():
    ...
    """
    if not _is_enabled():
        return None  # rezhim B — gard vyklyuchen
    if p2p_signature is None:
        return None  # biblioteka ne dostupna — fail-open (kak bylo)
    ...
    if err is None:
        # Zapomnim, esli klient ispolzoval legasi-aliyasy — vystavim deprecation-kheder v after_request
        used_legacy = False
    ...
    # Oshibka verifikatsii v†' edinyy otvet 401
...
def p2p_guard_after_request(response):
    """
    Na uspeshnykh zaprosakh dobavlyaem myagkuyu podskazku o postepennoy otmene legasi-khederov.
    """
    try:
