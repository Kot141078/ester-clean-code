...
Mosty:
- Yavnyy: (Alias ↔ Realnye routy) — probuem ester.routes.forms_routes i routes.forms_routes.
...
def register_forms_routes_alias(app, url_prefix: str = "/forms"):
    ...
    if not mod:
        # nechego registrirovat — tikho vykhodim
        return

    def do_register():
        # 1) Probuem shtatnuyu funktsiyu registratsii
        reg = getattr(mod, "register", None) or getattr(mod, "register_forms_routes", None)
        if callable(reg):
            try:
                reg(app)  # doveryaem originalu, guard perekhvatit konflikty pri B
                return
        ...
        if isinstance(bp, Blueprint):
            # esli takoy blueprint uzhe est — ne registriruem vtoroy raz
