...
MOSTY:
- (Yavnyy) after_app_request addavlyaet charset=utf-8 v Content-Type dlya JSON/HTML.
...
ZEMNOY ABZATs:
Na Windows PowerShell, esli server ne ukazhet kodirovku, otvet often
dekodiruetsya "ANSI/1251", otkuda i "ÐÑ...". This fayl garantiruet
...
def _patch_json_provider(app):
    # Flask >=2.2: ZHSON provider via class; softly override dumps()
    try:
        from flask.json.provider import DefaultJSONProvider  # type: ignore

        class Utf8JSON(DefaultJSONProvider):
            def dumps(self, obj, **kwargs):
                # Do not escape Cyrillic/Unicode
                kwargs.setdefault("ensure_ascii", False)
                # We don’t turn it on beautifully (fewer bytes); enable if desired:
        ...
        # Old Flask: just disable ASSI-escaping
        app.config["JSON_AS_ASCII"] = False

@bp.after_app_request
def _force_utf8(resp):
    # If the type is JSION - add a charset, if it is not there
    ctype = (resp.headers.get("Content-Type") or "").lower()
    if "application/json" in ctype and "charset=" not in ctype:
        resp.headers["Content-Type"] = "application/json; charset=utf-8"
    # Often useful for HTML/text as well
    if ctype.startswith("text/") and "charset=" not in ctype:
        resp.headers["Content-Type"] = (resp.headers["Content-Type"] + "; charset=utf-8").strip("; ")
    return resp

def register(app):
    # turn on the provider before any responses
    _patch_json_provider(app)
    # just mount the blueprint (there are no routes here) - so that the after_app_request will work
