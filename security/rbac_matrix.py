...
Sovmestimost:
- Nichego ne menyaet avtomaticheski. Vyzyvaetsya tolko esli app.py uzhe ispolzuet etot modul.
...
def _match_rule(path: str, rules: Dict[str, Any]) -> Optional[Tuple[str, Dict[str, Any]]]:
    """
    Vozvraschaet pervuyu podkhodyaschuyu zapis (pattern, rule).
    Prioritet — poryadok obyavleniya v YAML (dict s sokhraneniem poryadka v Py3.7+).
    """
...
def check_access(
    ...
    if not need:
        return True  # net yavnogo trebovaniya
...
def register_rbac_matrix(app, path: Optional[str] = None, extractor=None):
    ...
    def _rbac_matrix_guard():
        ...
        if not allowed:
            # Vozvraschaem 403 bez izmeneniya ostalnoy logiki prilozheniya.
            return (
