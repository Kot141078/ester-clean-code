...
def restore_full_config():
    ...
    # 1. Cleanup: We remove our previous “patches” so that there are no duplicates
    # We delete lines starting with our variables if they are already in the code
    # (This is a crude but effective way to clear hotfix junk)
    lines = content.splitlines()
    clean_lines = []
    
    # List of variables that we override
    vars_to_reset = [
    ...
    for line in lines:
        ...
        for v in vars_to_reset:
            # If the line begins with a variable declaration (for example, "DAC TIMEOUT =")
            # and this is NOT our new block (we haven't inserted it yet)
