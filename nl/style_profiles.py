...
ZEMNOY ABZATs:
Pishem “kak dlya lyudey”: yuristu - sukho i strukturno; shkolniku - simple;
drugu - warmth; po umolchaniyu — neytralno-delovoy.
...
def _ab() -> str:
    return os.getenv("AUTHORING_STYLE_AB", "A").upper()


def _persona() -> str:
    return os.getenv("MSG_STEALTH_PERSONA", "gentle")
...
def render_style(kind: str, intent: str, ctx: Dict | None = None) -> str:
    ...
    if kind == "lawyer":
        return _lawyer(intent, ctx)
    if kind == "student":
        return _student(intent, ctx)
    if kind == "friend":
        return _friend(intent, ctx)
